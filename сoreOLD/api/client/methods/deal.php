<?php
	...
	$client_id=$token_user[2];
		...
		$sql = $mysqli->query("SELECT * FROM `deals` WHERE client_id='".$client_id."' AND (closed_u!='1' OR closed_w!='1')");
		...
		while ($now_deal = mysqli_fetch_row($sql)){
					...
					echo "<div class='mdl-card mdl-cell mdl-cell--4-col mdl-shadow--4dp'>
						...
						<div class='mdl-card__actions mdl-card--border'>
							<button class=\"mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored \" onclick=\"accept_deal_c(".$now_deal[0].");this.disabled=1;find_deal_c();\" ".$disabled.">Сделка завершилась</button>
						</div>
						</div>";					
